...
### Timeout Cleanup in Finally Block (graduated: gene-20260320-timeout-finally)

- **Trigger**: 写 setTimeout/AbortController timeout 模式时
- **Action**: `clearTimeout` 必须放在 `finally` 块，不能只放 try 成功路径。错误路径跳过清理 = 静默资源泄漏（timer 保持 event loop 活跃，阻止进程退出）
- **Evidence**: `routes.ts` 两处 120s AbortController timeout，catch 路径不清 timer → jest worker 挂起 120s。修为 finally 块后干净退出
