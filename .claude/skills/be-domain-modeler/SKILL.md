...
### Plugin Config ≠ Global Config (graduated: gene-20260331-plugin-config-not-global)

- **Trigger**: OpenClaw 插件通过 `api.config` 访问 channel credentials
- **Action**: 不假设 `api.config` 含全局 channel credentials——plugin config 是插件作用域。无 token 时 fallback 读 `~/.openclaw/openclaw.json`
- **Evidence**: deliverInbound 从 plugin config 读 botToken 为空，token 仅在全局 openclaw.json。所有 bridge 投递静默失败数小时
