...
**Task**: Verify WeChat delivery truth
...
- OpenClaw WeChat runtime / bridge integration code
...
- Given: real WeChat send path code or runtime proof
  When: delivery confirmation is assessed
  Then: the evidence ends with exactly one line: `YES` or `NO`

- Given: the evidence file
  When: reviewed
  Then: it includes code location or runtime proof showing whether WeChat can prove end-user-visible delivery
...
- Do not treat "send returned without throw" as proof of delivery unless the channel explicitly confirms user-visible receipt
